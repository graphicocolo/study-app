...
export default function SplitCalculator() {
  const { total, nop, resultBase, resultRemainder, totalError, nopError, handleTotalChange, handleNopChange, handleBlurTotal, handleBlurNop, handleSubmit, handleReset } = useSplitCalculator()
      ...
      <form className="mb-4" onSubmit={handleSubmit} onReset={handleReset}>
        <div className="mt-6 space-y-2">
          <div className="flex flex-col">
            <label htmlFor="total" className="block text-base font-medium text-gray-700 mb-1 text-left">
          総額
            </label>
            <input
              type="number"
              ...
              onBlur={(e) => handleBlurTotal(e.target.value)}
            />
          </div>
          {totalError && <p className="text-red-500 mb-4">{totalError}</p>}
          <div className="flex flex-col">
            <label htmlFor="nop" className="block text-base font-medium text-gray-700 mt-4 mb-1 text-left">
          人数
            </label>
            <input
              type="number"
              ...
              placeholder="例: 3"
              value={nop}
              onChange={(e) => handleNopChange(e.target.value)}
              onBlur={(e) => handleBlurNop(e.target.value)}
            />
          </div>
